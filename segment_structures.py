        ...
        for fname in files:
            ...
            try:
                ...
                if uid not in series_meta:
                    snum = str(int(getattr(ds, "SeriesNumber", 0) or 0))
...
def find_dicom_folder(name):
    ...
    Predates ct_scans/; resolve_folders looks under DATA_ROOT before falling back
    here, so this only covers a loose DICOM* export sitting in the repo root."""
    cwd = Path.cwd()
...
def extract_and_cleanup_zips(folder):
    """Extract any .zip files directly in `folder` into subfolders and delete originals.
    Each `foo.zip` becomes `foo/`. Invalid zip files are logged and left alone."""
    folder = Path(folder)
    ...
    if not zips:
        return
    print(f"\nFound {len(zips)} zip file(s) in '{folder}' - extracting...")
    for zpath in zips:
        extract_to = folder / zpath.stem
        if extract_to.exists():
            print(f"  Skipping '{zpath.name}' - target '{extract_to.name}' already exists.")
            continue
        try:
            with zipfile.ZipFile(zpath, "r") as zf:
                zf.extractall(extract_to)
            print(f"  Extracted '{zpath.name}' -> '{extract_to.name}/'")
            zpath.unlink()
            print(f"  Removed original: {zpath.name}")
        except zipfile.BadZipFile:
            print(f"  Warning: '{zpath.name}' is not a valid zip file - skipping.")
        except Exception as e:
            print(f"  Error extracting '{zpath.name}': {e}")


def resolve_inputs(group, cases, folders, group_name):
    """Turn --group/--case into the folder list, so a group is named once.

    Naming the group twice - once as a path and again as --group-name - is what the
    positional form forces, and it is easy to get half right. Raises SystemExit with
    the reason rather than returning something empty that fails later and further
    away."""
    if cases and not group:
        raise SystemExit("--case needs --group")
    if group:
        gdir = group_dir(group)
        if not gdir.is_dir():
            raise SystemExit(f"no such group: {gdir}")
        if cases:
            missing = [c for c in cases if not (gdir / c).is_dir()]
            if missing:
                raise SystemExit(f"not in {gdir}: {', '.join(missing)}")
            folders = [str(gdir / c) for c in cases]
        else:
            folders = [str(gdir)]
        group_name = group_name or group
    if not folders:
        raise SystemExit("say what to segment: --group NAME, or a folder path")
    return folders, group_name


def resolve_folders(folder):
    """
    Given a path, return a list of (dicom_folder, group_name) tuples to process.
    group_name is the basename of the top-level input path — used to build output paths.
    - If it contains DICOM files directly -> single folder, group = its name
    - If subdirs contain DICOMs directly (series subfolders) -> single study, group = its name
    - If subdirs contain further subdirs -> batch; each subdir is a study, group = parent name
    - Otherwise, try find_dicom_folder() as fallback.
    """
    p = Path(folder)
    # Scans live under ct_scans/ now, so a bare group or group/case name - which is
    # what anyone types, and what the older docs show - no longer resolves from the
    # repo root. Try there before giving up on it.
    if not p.is_dir() and not p.is_absolute() and (DATA_ROOT / folder).is_dir():
        p = DATA_ROOT / folder
    group = p.name

    if p.is_dir():
        extract_and_cleanup_zips(p)

    if not p.is_dir():
        found = find_dicom_folder(folder)
        return [(found, found.name)] if found else []
    ...
    if subdirs_with_dicoms and not subdirs_with_subdirs:
        ...
        return [(sub, group) for sub in sorted(subdirs)]
...
def plan_all_folders(all_folders, cache, tasks, skip_manual=False, force_redo=False):
    ...
    for dicom_folder, group in all_folders:
        ...
        seg_out = seg_dir_for(group) / dicom_folder.name
        tasks_to_run = []
        for task in tasks:
            if force_redo:
        ...
        if best_score >= AUTO_SELECT_MIN_SCORE and gap >= AUTO_SELECT_MIN_GAP:
            ...
            resolved.append((dicom_folder, group, best_files, desc, snum, tasks_to_run))
            continue
...
def multilabel_to_segnrrd(seg_img, label_map, output_path, colors=None):
    ...
    Left/right paired structures share a color. `colors` optionally maps structure
    name -> "r g b" (0-1 floats as a string) to override the hash-derived color."""
...
def process_multilabel_output(multilabel_path, nifti_path=None, task_name=None, delete_source=True):
    """Convert a multilabel NIfTI from TotalSegmentator into a .seg.nrrd file and
    compute per-class statistics. If delete_source=True, removes the intermediate .nii.gz."""
    ...
    # --- Export individual structure NIfTIs into a subfolder ---
    structures_dir = multilabel_path.parent / stats_name
    ...
    for label_value, structure_name in label_map.items():
        ...
        if label_int == 0:
            continue
        mask = (seg_data == label_int).astype(np.uint8)
        if not mask.any():
            continue
        safe_name = sanitize_filename(structure_name)
        out_path = structures_dir / f"{safe_name}.nii.gz"
        nib.save(nib.Nifti1Image(mask, seg_img.affine, seg_img.header), out_path)
    print(f"  Individual structures saved to: {structures_dir}/")
...
def find_source_nifti(group, dicom_folder_name):
    """Return the path to the converted NIfTI for a study, if one exists."""
    nifti_out = nifti_dir_for(group) / dicom_folder_name
...
def main():
    parser = argparse.ArgumentParser(
        usage="%(prog)s (--group NAME [--case CASE ...] | FOLDER ...) --task TASK ...",
    ...
    parser.add_argument("dicom_folders", nargs="*",
                        help="DICOM folder(s) or parent folder containing subfolders. "
                             "Usually easier to name the group with --group instead.")
    parser.add_argument("--group", default=None,
                        help="study group to process, e.g. --group STUDY. Finds the "
                             "folder under ct_scans/ and names the outputs after it, "
                             "so the group is said once instead of as both a path and "
                             "a --group-name.")
    parser.add_argument("--case", nargs="+", default=None, metavar="CASE",
                        help="one or more case folders inside --group; omit for the "
                             "whole group")
    ...
    parser.add_argument("--nrrd", action="store_true",
                        help="Skip segmentation; only convert existing multilabel .nii.gz files to .seg.nrrd (and stats).")
    ...
    parser.add_argument("--roi-subset", "-rs", nargs="+", default=None, metavar="STRUCTURE",
                        help="Only segment specific structures (e.g. -rs brain lung_upper_lobe_left). "
                             "Only works with total/total_mr tasks. Much faster than segmenting everything.")
    ...
    all_folders = []
    args.dicom_folders, args.group_name = resolve_inputs(
        args.group, args.case, args.dicom_folders, args.group_name)
    ...
    if args.group_name:
        all_folders = [(f, args.group_name) for f, _ in all_folders]
    ...
    if args.stats_only:
        ...
        for dicom_folder, group in all_folders:
            ...
            seg_out = seg_dir_for(group) / name
            if not seg_out.is_dir():
                print(f"\nNo segmentation output found for '{name}' - skipping.")
                continue

            source_nifti = find_source_nifti(group, name)
            ...
            for ml_file in multilabel_files:
                ...
                process_multilabel_output(ml_file, source_nifti, task_name=task_name, delete_source=False)
        return

    # --nrrd: convert existing multilabel .nii.gz files to .seg.nrrd and delete the source
    if args.nrrd:
        print("\n" + "=" * 60)
        print("NRRD CONVERSION MODE")
        print("=" * 60)
        for dicom_folder, group in all_folders:
            name = Path(dicom_folder).name
            seg_out = seg_dir_for(group) / name
            ...
            for ml_file in multilabel_files:
                ...
                if task_name.endswith(".nii.gz"):
                    ...
                    task_name = task_name[:-4]
                if (seg_out / f"{task_name}.seg.nrrd").exists():
                    print(f"  [SKIP] {task_name}.seg.nrrd already exists")
                    continue
                print(f"  {ml_file.name}")
                process_multilabel_output(ml_file, source_nifti, task_name=task_name, delete_source=True)
        return
    ...
    for dicom_folder, group, files, desc, snum, tasks_to_run in work_items:
        try:
            print(f"\n{'=' * 60}")
            print(f"Processing: {dicom_folder}  (group: {group})")
            ...
            nifti_out = nifti_dir_for(group) / dicom_folder.name
